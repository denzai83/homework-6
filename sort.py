    ...
    for root, dirs, files in os.walk(basepath):
        for file in files:
            if os.path.splitext(file)[1].lower() in CATEGORIES["images"]:
                ...
                os.replace(f"{root}/{file}",
                           f"{basepath}/Audio/{normalize(file)}")
            elif os.path.splitext(file)[1].lower() in CATEGORIES["video"]:
                os.replace(f"{root}/{file}",
                           f"{basepath}/Video/{normalize(file)}")
            elif os.path.splitext(file)[1].lower() in CATEGORIES["archives"]:
                os.replace(f"{root}/{file}",
                           f"{basepath}/Archives/{normalize(file)}")
                shutil.unpack_archive(f"{basepath}/Archives/{file}",
                                      f"{basepath}/Archives/{os.path.splitext(file)[0]}")
